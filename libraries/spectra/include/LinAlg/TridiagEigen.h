...
public:
    ...
    inline bool isMuchSmallerThan(
        ...
        typename Eigen::NumTraits<Scalar>::Real precision =
            Eigen::NumTraits<Scalar>::dummy_precision()) {
      return Eigen::numext::abs2(x) <= Eigen::numext::abs2(y) * precision * precision;
    }

    inline static void tridiagonal_qr_step(RealScalar *diag,
    ...
    void compute(ConstGenericMatrix &mat)
    {
        ...
        while(end > 0)
        {
            for(int i = start; i < end; i++)
                if(isMuchSmallerThan(std::abs(subd[i]), (std::abs(maind[i]) + std::abs(maind[i + 1]))))
                    subd[i] = 0;
